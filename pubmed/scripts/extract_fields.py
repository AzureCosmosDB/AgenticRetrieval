...
ALL_PAPERS = set([p.split("/")[-1].replace(".xml", "") for p in xml_files])

# Pre-load tables into memory with indexes
con = duckdb.connect()
...
def filter(refs):
    ...
    if not refs:
        return []
    ref_pmids = ",".join(f"'{r}'" for r in refs)
    matches = con.sql(f"SELECT PMCID FROM pmc_ids WHERE PMID IN ({ref_pmids})").fetchall()
    ref_filtered = [r[0] for r in matches if r[0] in ALL_PAPERS]
    return ref_filtered

def find_citing_and_cited_papers(pmcid):
    row = con.sql(f"SELECT PMID FROM pmc_ids WHERE PMCID = '{pmcid}' LIMIT 1").fetchone()
    if row is None or row[0] is None:
        return [], []
    pmid = row[0]

    cite = con.sql(f"SELECT PMID FROM refs WHERE RefPMID = {pmid}").fetchall()
    # print(f"Number of papers that cite PMCID {pmcid}: total {len(cite)}")

    cited = con.sql(f"SELECT RefPMID FROM refs WHERE PMID = {pmid}").fetchall()
    # print(f"Number of papers cited BY PMCID {pmcid}: total {len(cited)}")

    return filter(cite), filter(cited)
...
results = []
for filepath in tqdm(xml_files[:1000]):
